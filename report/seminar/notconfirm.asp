...
%>
<p class="bheader">Seminar Totals - Suppliers not Accepted Yet</p>
<%
										else
%>
<p class="bheader">Seminar Totals - Suppliers Invited</p>										
<%										
										end if
										...
										if Request.QueryString("type") = "1" then
											...
											Set ReturnSet = ExecuteSql("listNotConfirmSupplier @InviteDate=" & MakeSQLDate(TestDate), curConnection)   
										else
											' Call the sp - listSupplierInvited
											Set ReturnSet = ExecuteSql("listSupplierInvited @InviteDate=" & MakeSQLDate(TestDate), curConnection)  
										end if
										'response.write "exec listNotConfirmSupplier @InviteDate=" & MakeSQLDate(TestDate)
										'response.end
