...
<xsl:template match="/" xml:space="preserve">
	<!--<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr>
			...
			<td class="NavLink" bgcolor="#333366" align="center"><a href="@@ApplicationRoot/track/supplier/frmcontent.asp?action=2&amp;id=@@Date" class="NavLink" target="frmcontent">Electronic Invoices</a>@@GenInv
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			...
			<table border="0" cellpadding="2" cellspacing="2" width="100%">
				 <xsl:apply-templates select="rootnode/spmessage/supplier/store"/>
			</table>
		</xsl:otherwise>
	</xsl:choose>
</xsl:template>
...
<!--<xsl:template match="supplier">
	<tr>
		<td>&#160;</td>
		<td class="mheader" colspan="11" bgcolor="#ccccc2"><xsl:value-of select="name"/></td>
	</tr>
...
<xsl:template match="invoice">	
	<tr>
		<td class="pcontent" align="center"><a class="links" href="JavaScript: newWindow = openWin('@@ApplicationRoot/track/supplier/invoice/default.asp?item={@id}', 'InvoiceDetail', 'width=700,height=500,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=1,resizable=1');"><xsl:value-of select="invoicenumber"/></a></td>
		...
		<xsl:choose>
			<xsl:when test="ClaimNumber!=''">
				<td class="pcontent" align="center"><a class="links" href="JavaScript: newWindow = openWin('@@ApplicationRoot/track/supplier/claim/default.asp?item={ClaimId}', 'ClaimDetail', 'width=800,height=500,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=1,resizable=1');"><xsl:value-of select="ClaimNumber"/></a></td>				
			</xsl:when>
			<xsl:otherwise>
				<td class="pcontent" align="center">-</td>
			</xsl:otherwise>
		...
		<xsl:choose>
			<xsl:when test="CreditNoteNumber!=''">
				<td class="pcontent" align="center"><a class="links" href="JavaScript: newWindow = openWin('@@ApplicationRoot/track/supplier/creditnote/default.asp?item={CreditNoteId}', 'CreditNoteDetail', 'width=1200,height=900,toolbar=0,location=0,directories=0,status=1,menuBar=0,scrollBars=1,resizable=1');"><xsl:value-of select="CreditNoteNumber"/></a></td>							
			</xsl:when>
			<xsl:otherwise>
				<td class="pcontent" align="center">-</td>
			</xsl:otherwise>
