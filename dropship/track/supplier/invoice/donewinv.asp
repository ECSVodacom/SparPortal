...
<%
										...
										'if Request.Form("hidAction") = "1" then
											...
											strXMLHead = strXMLHead & "<DATE1>" & LZ(YEAR(Request.Form("txtInvoiceDate"))) & LZ(MONTH(Request.Form("txtInvoiceDate")))& LZ(DAY(Request.Form("txtInvoiceDate"))) & "</DATE1>"
											...
											strXMLHead = strXMLHead & "<REFN>" & Request.Form("txtInvoiceNo") & "</REFN>" 
											strXMLHead = strXMLHead & "<DATE>" & LZ(YEAR(Request.Form("txtInvoiceDate"))) & LZ(MONTH(Request.Form("txtInvoiceDate")))& LZ(DAY(Request.Form("txtInvoiceDate"))) & "</DATE>"
											...
											For LCount = 1 to Request.Form("hidTotalCount")
												if Request.Form("hidChkDelete" & LCount) = "0" Then
													...
													if Request.Form("txtSupPack" & LCount) <> "" or Round(Request.Form("txtSupPack" & LCount)) <> "0" then
														NetConsum = Round(CDbl(Request.Form("txtListCost" & LCount)) / CDbl(Request.Form("txtSupPack" & LCount)),4)
													else
														NetConsum = 0
													end if
													...
													strXMLLine = strXMLLine & "<NCCP>" & NetConsum & "</NCCP>"
											...
											if ReturnSet("returnvalue") <> 0 then
												...
												'Response.Redirect const_app_ApplicationRoot & "/track/supplier/invoice/new.asp"
												'Response.Flush
												Response.Write const_app_ApplicationRoot & "/track/supplier/invoice/new.asp"
											else
												...
												Response.Redirect const_app_ApplicationRoot & "/track/supplier/invoice/default.asp?item=" & InvoiceID & "&success=1"
												Response.Flush
												'Response.Write const_app_ApplicationRoot & "/track/supplier/invoice/default.asp?item=" & InvoiceID & "&success=1"
											end if
										'else
%>
