...
<%
										...
										' Open the text file
										'esponse.Write const_app_IncludePath & Folder & "<br/>"
										'Response.Write const_app_IncludePath
										'C:\InetPub\wwwroot\Spar\dropship\includes\
										'Response.Write const_app_IncludePath & Folder & "<br/>"
...
<frameset  rows="117,*" border="0">
	...
	<frameset cols="150,10,*" border="1" onload="reDisplay('0',true)">
		...
		<!--</frameset>-->
		<frame name="frmMiddle" src="frmmiddle.asp" scrolling="no" noresize ></frame>
<%			
...
%></frame>
		<frame name="frmcontent" src="frmcontent.asp?id=<%=FormatDate(CDate(NewDate),false)%>&RRID=<%=request.querystring("RRID")%>" scrolling="auto" noresize>
<%													
												end if
											else
%></frame>
		<frame name="frmcontent" src="<%=const_app_ApplicationRoot%>/storeval/default.asp?exceptid=<%=request.querystring("exceptid")%>" scrolling="auto" noresize>
<%											
											end if
%>		
	</frameset>
