	...
	Function MakeXMLClaims (DBConnection, SQL)
		...
		if ReturnSet("returnvalue") <> 0 then
			...
			While not ReturnSet.EOF
				...
				MyString = MyString & "<cnid>" & ReturnSet("CreditNoteID") & "</cnid>"
...
'response.end
				...
				if LineSet("returnvalue") = 0 then
					While Not LineSet.EOF
						...
						if LineSet("proddescr") <> "" then
							MyString = MyString & "<proddescr>" & Replace(Replace(LineSet("proddescr"),"&","&amp;"),"<","&lt;") & "</proddescr>"
						else
		...
		'response.write(MakeCreditNoteItemXML)
		'response.end
		...
		if ReturnSet("returnvalue") <> 0 then
			...
			While not ReturnSet.EOF
				...
				if ReturnSet("SPcName") <> temp Then
					...
					hasSup = true
					MyString = MyString & "<supplier><name>" & Replace(temp,"&","&amp;") & "</name>"
				end if
				MyString = MyString & "<file>"
