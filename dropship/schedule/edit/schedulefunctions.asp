<%
	function ProcessExcel (dboConn, SupplierID, UserID, DCID, FilePath, FileName, FileSize, IsForceCredit, IsRewardsNumeric,IsStampsNumeric)
		...
		Set objRS= ExecuteSql(strSQL, objConn) 
...
End If
	...
	function ProcessReleaseMessage(dboConn, HeaderID, OutPath)
		...
		Dim SqlCommand 
		SqlCommand = "exec itemSchedule @ScheduleID=" & HeaderID & ", @Release=1"
		...
		'response.end
	   	...
	   	Set ReturnSet =  ExecuteSql("exec editScheduleStatus @ScheduleID=" & HeaderID & ", @StatusID=5", curConnection)    
	   	Set Returnset = Nothing
