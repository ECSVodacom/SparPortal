<%
	function ProcessExcel (dboConn, SupplierID, UserID, DCID, FilePath, FileName, FileSize, IsForceCredit, IsRewardsNumeric, IsAdmin)
		...
		dIM FileType
		FileNameArray = Split(FileName,".")
		FileType = LCase(Trim(FileNameArray(UBound(FileNameArray))))
		'Response.Write const_app_uploaddir
		'If FileType = After2003 Then
			ConnectionString = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" & const_app_uploaddir & FileName & ";Extended Properties='Excel 12.0 Xml;HDR=No; IMEX=1'"
	'	ElseIf FileType = Before2003 Then
	'		ConnectionString = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & const_app_uploaddir & FileName & ";Extended Properties='Excel 8.0; HDR=Yes; IMEX=1'"
	'	Else
	'		ProcessExcel = "Invalid file type"	
	'		Exit Function
	'	End If
	...
	'	On Error Resume Next
        ...
        Set objRS=objConn.Execute(strSQL) ' This is for the excel driver
		...
		'ElseIf (IsStampsNumeric = 1 And objRS.Fields.count <> 10 ) Then
		'	ReturnVal = ReturnVal & "ERROR: The SPAR STAMPS schedule you are tying to upload does not match the file type selected or the file content is not in the agreed format."	
		'	Response.write "B"

		
		'ElseIf (IsStampsNumeric = 0 And IsRewardsNumeric = 0 And (objRS.Fields.count < 8 or objRS.Fields.count > 9)) Then
		'	ReturnVal = ReturnVal & "ERROR: The SPAR STAMPS schedule you are tying to upload does not match the file type selected or the file content is not in the agreed format."	
		'Response.write "C"
		ElseIf (objRS.Fields.count < 8 )  Then
	        ...
	        if err then
				...
				", @IsStampsNumeric =" & IsStampsNumeric & _
				", @IsAdmin=" & IsAdmin 
		'Response.write "E"
		'Response.Write SQL
		''Response.end
                Set ReturnSet =  dboConn.Execute(SQL) 'ExecuteSql(SQL, dboConn)    
                ...
                if ReturnSet("returnvalue") <> "0" then
                    ...
                    Do While Not objRS.EOF 
					   ...
					   if objRS.Fields.Item(4).Value <> "" then
                            amtIncl = Replace(objRS.Fields.Item(4).Value,",",".")
							
                        end if
                        
                        if objRS.Fields.Item(5).Value <> "" then
                            vatAmt = Replace(objRS.Fields.Item(5).Value,",",".")
                        end if
                        
                        if objRS.Fields.Item(6).Value <> "" then
                            amtExcl = Replace(objRS.Fields.Item(6).Value,",",".")
                        end if
						...
						' If StoreCode is not numeric then ignore
							...
							If IsStampsNumeric = 1 Then
								SQL = SQL & _
								", @TransactionType=" & MakeSQLText(objRS.Fields.Item(9).Value) 
                    ...
                    SQL = "editScheduleHeader @ScheduleID=" & scheduleID & _	
                        ", @Total=" & Total & _
...
End If
	...
	function ProcessReleaseMessage(dboConn, HeaderID, OutPath)
	    ...
	    Set ReturnSet = ExecuteSql(SqlCommand, dboConn) 'ExecuteSql(SQL, dboConn)     ExecuteSql(SqlCommand, dboConn) 
	        ...
	        While NOT (ReturnSet.EOF)
				...
				Dim ProductLine
					...
					ElseIF ReturnSet("IsStamps")then
						ProductLine = ReturnSet("SDcTransactionType")
					Else
                ...
                If CDbl(ReturnSet("AmtIncl")) > 0 Then
					...
					If ReturnSet("IsForceCredit") Then
						DcInvoiceForceCreditIndicator = "::FCR"
					End If
					...
					VatAmount = ReturnSet("VatAmt")
					AmountExclusive = ReturnSet("AmtExcl")
				    if VatAmount <> "0" then
						'VatPerc = 14
						VatPerc = CDbl(VatAmount) / CDbl(AmountExclusive) * 100
						VatPerc = Round(VatPerc)
				    end if
                    ...
                    InvVal = InvVal & "ODD+1+N/A::+" & DcInvoiceForceCreditIndicator & "+:" & FormatEDIDate(ReturnSet("DocDate"))  & "'" & vbcrlf
					'InvVal = InvVal & "ODD+1+N/A::++:" & FormatEDIDate(ReturnSet("DocDate")) & "'" & vbcrlf
                    InvVal = InvVal & "ILD+1++:::"&ProductLine&"+1:1+" & ReturnSet("AmtExcl") & ":1++" & ReturnSet("AmtExcl") & "++" & VatPerc & "+S+" & ReturnSet("AmtExcl") & "'" & vbcrlf
                ...
                Else
					...
					CredVal = CredVal & "CLO+" & Trim(ReturnSet("StoreEAN")) & "+" & ReturnSet("DCEAN") & "+" & StoreName & DcForceCreditIndicator & "'" & vbcrlf
                    ...
                    CredVal = CredVal & "LDS+1++" & ReturnSet("AmtIncl") & "+" & VatPerc & "+s++++00000000000000:::"&ProductLine&"+0+" & ReturnSet("AmtExcl") & "'" & vbcrlf
		...
		'response.write "exec editScheduleStatus @ScheduleID=" & HeaderID & ", @StatusID=5"
		'response.end

	   	' Update the status of this message to released
	   	Set ReturnSet =  curConnection.Execute("editScheduleStatus @ScheduleID=" & HeaderID & ", @StatusID=5")   ' ExecuteSql("editScheduleStatus @ScheduleID=" & HeaderID & ", @StatusID=5", curConnection)  
	   	Set Returnset = Nothing
    ...
    function GenCSVSavedFile (dboConn, HeaderID, OutPath)
        ...
        Set ReturnSet = dboConn.Execute("exec itemSchedule @ScheduleID=" & HeaderID) 'ExecuteSql("exec itemSchedule @ScheduleID=" & HeaderID, dboConn)   
