...
<%
	...
	If Request.Form("chkSelect") <> "" And  ButtonAction = "Delete" Then
		...
		For i = 0 To UBound(ClaimCategoryIds)
			SqlUpdate = "MaintainClaimCategory @ClaimCategoryId=" & Trim(ClaimCategoryIds(i)) & ", @Action=1, @ClaimTypeId=1" 			
...
<script type="text/javascript">
	...
	function OnDelete() {
		...
		for (var i = 0; i < SupplierAdminClaimsCategories.elements.length; i++ ) {
			if (SupplierAdminClaimsCategories.elements[i].type == 'checkbox') {
				if (SupplierAdminClaimsCategories.elements[i].checked == true) {
...
<form name="SupplierAdminClaimsCategories" method="post" action="SupplierAdminClaimsCategories.asp">
	...
	<table>
		<tr>
			<td>
				<input class="button" type="button" onclick="window.open('<%=const_app_ApplicationRoot%>/claims/SupplierAdminClaimsCategoriesMaintain.asp?ClaimCategoryId=0')" style="width: 98px" value="Add New"/>
			</td>
			<td>
...
<%	
	...
	Set rsObj =  ExecuteSql("ListClaimsCategories @ClaimTypeId=1, @ClaimCategoryType = 0", cnObj) 
...
%>	
		<table cellSpacing="2" cellPadding="4" border="0">
			<tr bgcolor="#4C8ED7">
				...
				<td class="tdcontent" align="center"><b>Warehouse Category Id</b></td>
				<td class="tdcontent" align="center"><b>Category Type</b></td>
			</tr>
...
<%
		...
		While Not rsObj.EOF
			...
			End If
			Response.Write "	<td class='pcontent'>"%><a href='<%=const_app_ApplicationRoot%>/claims/SupplierAdminClaimsCategoriesMaintain.asp?claimcategoryid=<%=rsObj("ClaimCategoryId")%>' target='_blank'><%=rsObj("ClaimCategory")%></a></td><%
			Response.Write "	<td class='pcontent'>" & ActiveInactiveIndicator & "</td>"
			Response.Write "	<td class='pcontent'>" & rsObj("WarehouseCategoryId") & "</td>"
			Response.Write "	<td class='pcontent'>" & rsObj("CategoryType") & "</td>"
			Response.Write "</tr>"
...
%>
		<table border="1" cellpadding="0" cellspacing="0" width="50%">
			<tr bgcolor="#4C8ED7">
				<td class="tdcontent" align="center">No supplier claims categories available</td>
			</tr>
		</table>
<%	
	End If
