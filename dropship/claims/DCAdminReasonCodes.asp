...
<script type="text/javascript">
	...
	function OnDelete() {
		...
		for (var i = 0; i < DCAdminReasonCodes.elements.length; i++ ) {
			if (DCAdminReasonCodes.elements[i].type == 'checkbox') {
				if (DCAdminReasonCodes.elements[i].checked == true) {
...
<form name="DCAdminReasonCodes" method="post" action="DCAdminReasonCodes.asp">
	...
	<table>
		<tr>
			<td>
				<input class="button" type="button" onclick="window.open('<%=const_app_ApplicationRoot%>/claims/DCAdminReasonCodeMaintenance.asp?ClaimReasonId=0')" style="width: 98px" value="Add New"/>
			</td>
			<td>
...
<%	
	...
	Set rsObj = ExecuteSql("ListClaimsCategoriesReasonCodes @ClaimTypeId=2", cnObj)       
...
<%
		...
		While Not rsObj.EOF
			...
			If (ParentIsActive ) Then
				...
				Response.Write "	<td class='pcontent'>"%><a href='<%=const_app_ApplicationRoot%>/claims/dcadminreasoncodemaintenance.asp?ClaimReasonId=<%=rsObj("ClaimReasonId")%>' target='_blank'><%=rsObj("ClaimReasonDescription")%></a></td><%
			...
			Else
				...
				Response.Write "	<td class='pcontent'>" & rsObj("ClaimCategory") & "</td>"
				Response.Write "	<td class='pcontent'>" & rsObj("ReasonCode") & "</td>"
				Response.Write "	<td class='pcontent'>" & rsObj("ClaimReasonDescription") & "</td>"
				Response.Write "	<td class='pcontent'>" & ActiveInactiveIndicator & "</td>"
			End If
			Response.Write "</tr>"
