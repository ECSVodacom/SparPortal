...
Function GenMail (FromAddress, ToAddress, CCAddress, BCcAddress, Subject, BodyText, Importance, MailFormat, BodyFormat)
	...
	Command = "exec sp_send_cdosysmail " _
		& "@From='" & FromAddress _
		& "', @To='" & ToAddress _
		& "',@Cc='" &  CCAddress  _
...
Function AddToLog (LogName, ArchDate)
	...
	'Set aFileText = aFile.OpenTextFile("C:\Inetpub\wwwroot\spar\dropship\" & ArchDate & ".log",8,True)
...
Function GenTigerXML (XMLObject, XMLFile, FilePath, AppRoot)
	...
	For ItemCount = 0 to LineItem.Length-1
   	...
   	Next
		...
		XSLDoc.Load ("\\ecintprd\ec-clients\spar\FTP_CLIENTS\TigerBrands\map\tyger.xsl")
...
Function GenAdcockXML (XMLObject, XMLFile, FilePath, AppRoot)
	...
	' Changed by Petrus for Wintell Migration 29 November 2010\
	oOut.SaveToFile "\\ecftpprd\ftproot\LocalUser\adcockftp\Out\SPARPH" & NewPad & NewCount & ".xml"
'	oOut.SaveToFile "Z:\SPARPH" & NewPad & NewCount & ".xml"
...
Function Main()
	...
	const const_db_ConnectionString = "Provider=SQLOLEDB.1;Persist Security Info=True;User Id=SparUser;Password=ECsqlOnline!;Initial Catalog=Spar;Data Source=192.168.101.36,1433"
	const const_app_ApplicationRoot = "https://spar.gatewayec.co.za/"
	const const_app_TigerPath = "\\ecintprd\ec-clients\spar\FTP_CLIENTS\TigerBrands\"
	const const_app_AdcockPath = "\\ecintprd\ec-clients\spar\FTP_CLIENTS\Adcock\Spar\"
	const const_app_Path = "\\ecintprd\ec-clients\spar\"
	const const_app_DistellOut = "\\ECFTPPRD\ftproot\LocalUser\6004930011314\out\"
	const const_app_ObjXML = "MSXML2.DomDocument"
	const const_app_MailFrom = "edisupport@vodacom.co.za"
	const const_app_MailTo = "edisupport@vodacom.co.za"
	const const_app_MailCC = "petrus.daffue@vodacom.co.za"
	const const_app_Error = "danny.leshaba@vodacom.co.za;wendy.cuthill@vodacom.co.za;xander.barnard@vcontractor.co.za"

	' Set the FolderName
	FolderName = "C:\Inetpub\wwwroot\spar\dropship\error\"
	...
	if Files_Collection.Count > 0 Then
		...
		For each File in Files_Collection
			...
			if objGetFile.Size > 0 Then
				...
				'	if Not oFile.FolderExists("C:\Inetpub\wwwroot\spar\dropship\error\" & ArchiveDate) Then
						' Create the folder
				'		oFile.CreateFolder ("C:\Inetpub\wwwroot\spar\dropship\error\" & ArchiveDate)
				'	end if
			...
			'		oFile.MoveFile const_app_Path & "SparIn\" & FileName ,const_app_Path & "SparErrors\Orders\" & ArchiveDate & "\" & FileName
					...
					if Returnset("returnvalue") <> 0 Then
						...
						if objXML.selectSingleNode("//DOCUMENT/UNB/UNH/recievedate").text = "" then
							objXML.selectSingleNode("//DOCUMENT/UNB/UNH/recievedate").text = Year(now) & "/" & month(now) & "/" & day(now) & " " & FormatDateTime(Now,3)
						end if

						if objXML.selectSingleNode("//DOCUMENT/UNB/UNH/translatedate").text = "" then
							objXML.selectSingleNode("//DOCUMENT/UNB/UNH/translatedate").text = Year(now) & "/" & month(now) & "/" & day(now) & " " & FormatDateTime(Now,3)
						end if
						...
						if ReturnSet("returnvalue") <> 0 Then
							...
							For LineCounter = 0 to LineItem.Length-1
								...
								if SupplierEAN = "DISTILLERS001" OR SupplierEAN = "DISTELLERSWC1" OR SupplierEAN = "DISTELLERSSR1"  OR SupplierEAN = "DISTELLERSNR1"  OR SupplierEAN = "DISTELLERSEC1" Then
									' Save XML File to the new folder
									objXML.save(const_app_DistellOut  & OrderNumber & ".xml")

								end if	
									
								' Check if this is a order for Tiger Brands
								if SupplierEAN = "6004930003319" or SupplierEAN = "6004930006310" or SupplierEAN = "6001231000108" or SupplierEAN = "6004930005351" or SupplierEAN = "6001275000010" or SupplierEAN = "6001206428890" or SupplierEAN = "6001319000013"  or SupplierEAN = "6001479000007" Or SupplierEan = "6009684860003" Then
								...
								if NumExist = 0 Then
									strOrderNumber = split(OrderNumber,"s")

									' Build the subject line and BodyText for the supplier
									StrSubject = "Purchase Order " & strOrderNumber(0) & " from " & GetDC(BuyerEAN) & " for " & SupplierName
										
									if IsNumeric(SupplierEAN) Then
										BodyText = "<html><body><p><font face='Arial' size='2'>This e-mail contains information on Purchase Orders sent via Vodacom Business.</font></p>" 
										BodyText = BodyText & "<p><font face='Arial' size='2'>Please do not reply to this e-mail.</font></p>"
										BodyText = BodyText & "<p><font face='Arial' size='2'>Purchase Order " & strOrderNumber(0)  & " was sent from " & GetDC(BuyerEAN) & " and has been placed in your mailbox.</font></p>"
									...
									Call GenMail(BuyerEMail, SupplierEMail, CCAddress, const_app_MailTo, StrSubject, BodyText, 2, 0, 0)
								else
									...
									if SupplierEMail = "" or ISNULL(SupplierEmail) then
										SupplierEMail = "edisupport@vodacom.co.za"
									end if
									
									SupplierMailSplit = Split(SupplierEmail,";")
									Call GenMail(SupplierMailSplit(0), BuyerEMail, CCAddress, const_app_MailTo, StrSubject, BodyText, 1, 1, 1)
								end if
						end if
					end if
