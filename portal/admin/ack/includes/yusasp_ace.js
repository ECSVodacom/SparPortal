...
function getPageCSSText()
	...
	if(this.getPageProperties("backgroundImage")=="none"){sTmp=""}
		...
		{
		//Change : background-image:url("includes/bullet.gif")
		//To :     background-image:url('includes/bullet.gif')
		sTmp = this.getPageProperties("backgroundImage");
		sTmp=(sTmp.substr(5));//remove first col("
		sTmp = sTmp.substr(0,sTmp.length-2);//remove last ")
		sTmp = "url('" + sTmp + "')";
		...
		if(oEl)
			...
			if(inpAnchor!="")
				...
				if(isImgAnchorExist!=true)
					oEl.innerHTML = "<IMG src='../includes/yusasp_anchor.gif' align=middle border=0>" + oEl.innerHTML;
				}
			else
				{
				for(var j=0;j<oEl.childNodes.length;j++)
...
function InsertAsset_Flash(inpURL, inpFlashWidth, inpFlashHeight)
	...
	For example :
	this.baseEditor = "https://spar.gatewayec.co.za/ACE/AdvContentEditor55_StaticFiles/"
	this.base = "https://spar.gatewayec.co.za/ACE/AdvContentEditor55_StaticFiles/Files/Files2/"
	inpURL (APPLIED_LINK) = "files/tes.gif" (seen from this.baseEditor)
		or => https://spar.gatewayec.co.za/ACE/AdvContentEditor55_StaticFiles/files/tes.gif
	...
	for(var i=0;i<oEditor.document.all.length;i++)
		...
		if(oEditor.document.all[i].tagName=="A")
			...
			if(oEditor.document.all[i].name!="")
				oEditor.document.all[i].innerHTML = "<IMG src='../includes/yusasp_anchor.gif' align=middle border=0>" + oEditor.document.all[i].innerHTML;
			}
		}
	//~~~~~~~~~~~~~
	}
...
function RUN()
	...
	if(this.usePageProperties && !document.getElementById("boxPage"))//boxPage
		{
		document.write("<iframe id=boxPage name=boxPage src='../includes/box_Page.htm' style='position: absolute; visibility: hidden; z-index: -1;width:1;height:1;'></iframe>");
		}

	if(this.useTable && !document.getElementById("boxTable2"))//boxTable
		{
		document.write("<iframe id=boxTable2 name=boxTable2 src='../includes/box_Table.htm' style='position: absolute; visibility: hidden; z-index: -1;width:1;height:1;'></iframe>");
		}

	if(this.useSymbol && !document.getElementById("boxSymbol"))//boxSymbol
		{
		document.write("<iframe id=boxSymbol name=boxSymbol src='../includes/box_Symbol.htm' style='position: absolute; visibility: hidden; z-index: -1;width:1;height:1;'></iframe>");
		}
	}
...
function ACEditor(oName)
	...
	this.iconFolder = "../includes/images/";
...
function getEditorPreviewPath(APPLIED_LINK)//APPLIED_LINK=> Relative
	{
	//kalo absolute (ada http://, http://... tdk diproses
