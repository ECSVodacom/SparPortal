...
%>
		...
		<!--
		var appRoot = "http://10.34.50.1/spar/portal"
		if (!window.confirm("Are you sure you want to log out?")){
			window.location.href=appRoot + "/track/frmcontent.asp?id=Home";
		};
		...
		function Send_onclick(frmSubmit) {
			var const_app_ApplicationRoot = 'http://10.34.50.1/spar/portal';
			startclock();
			
			for (var i=0;i<=2000000;i++) {
				if (i==2000000) {
					self.parent.location.href=const_app_ApplicationRoot+'/logout/default.asp';		
				};
			};
		}
