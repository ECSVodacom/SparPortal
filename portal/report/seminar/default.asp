...
<p class="bheader">Seminar Totals</p>
<%										
										' Create a connection
										...
										Set ReturnSet = ExecuteSql("procSeminarTotal", curConnection) 
...
%>
	<tr>
		...
		<td class="tbldata" align="center"><a class="diffnav" href="<%=const_app_ApplicationRoot%>/report/seminar/notconfirm.asp?id=<%=ReturnSet("InviteDate")%>&type=2"><%=ReturnSet("TotInvited")%></a></td>
		<td class="tbldata" align="center"><%if ReturnSet("TotUnConfirm") <> 0 then%><a class="diffnav" href="<%=const_app_ApplicationRoot%>/report/seminar/notconfirm.asp?id=<%=ReturnSet("InviteDate")%>&type=1"><%=ReturnSet("TotUnConfirm")%></a><%else Response.Write ReturnSet("TotConfirm") end if%></td>
		<td class="tbldata" align="center"><%if ReturnSet("TotConfirm") <> 0 then%><a class="diffnav" href="<%=const_app_ApplicationRoot%>/report/seminar/confirm.asp?id=<%=ReturnSet("InviteDate")%>&type=1"><%=ReturnSet("TotConfirm")%></a><%else Response.Write ReturnSet("TotConfirm") end if%></td>
	</tr>
<%												
												ReturnSet.MoveNext
											Wend
