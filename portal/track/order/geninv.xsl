...
<xsl:choose>
	...
	<xsl:otherwise>
	<form action="dogeninv.asp?item=@@OrdID" method="post" name="frmInvoice" id="frmInvoice" onsubmit="return validate(this);">
 ...
 <xsl:template match="ordline" >
 	<tr>
 		<td class="pcontent" align="center"><xsl:value-of select="position()"/></td>
		<td class="pcontent" align="left">
			...
			<input type="text" name="txtOrdCode{position()}" id="txtOrdCode{position()}" size="15" value="" class="tblcontent"/><br/>
		...
		<td class="pcontent" align="center"><input type="text" name="txtMeasure{position()}" id="txtMeasure{position()}" size="2" class="tblcontent"/></td>
