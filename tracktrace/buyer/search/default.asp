...
<!--
	if (<%=Session("IsLoggedIn")%> != 1) {
		top.location.href = "<%=const_app_ApplicationRoot%>/tracktrace";
	};
...
<%
										...
										if Request.Form("hidAction") = "1" or Request.QueryString("page") <> "" Then
											...
											' Build the SQL
											SQL = "procSearch @OrderNumber=" & MakeSQLText(Session("OrderNumber")) & _
												", @SupplierID=" & Session("Suppl") & _
												", @RecordBand=" & Band & _
												", @Permission=" & Session("Permission") & _
												", @ProcID=" & Session("ProcID") & _
...
<table border="0" cellpadding="2" cellspacing="2">
	<tr>
		<td class="shead" align="left">Search Results</td>
	</tr>
	<tr>
...
<%												
												...
												Call PageNav (const_app_ApplicationRoot & "/tracktrace/buyer/search/default.asp", "pcontent", RecordCount, MaxRecords, RecordFrom, RecordTo, TotPages, Band)
%>
...
%>
				<b><%=Replace(ReturnSet("OrderNumber"),Right(ReturnSet("OrderNumber"),4),"")%></b><% Response.Write(" ")%><a class="textnav" target="_blank" href="<%=const_app_ApplicationRoot%>/orders/supplier/default.asp?id=<%=ReturnSet("XMLRef")%>&amp;type=1&check=0&doAction=view">View</a>
<%					
													else
%>
			<a class="textnav" target="_blank" href="<%=const_app_ApplicationRoot%>/orders/buyer/default.asp?id=<%=ReturnSet("XMLRef")%>&amp;type=1"><%=Replace(ReturnSet("OrderNumber"),Right(ReturnSet("OrderNumber"),4),"")%></a>
<%													
													end if
%>			
