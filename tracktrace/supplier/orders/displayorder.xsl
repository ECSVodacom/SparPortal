...
<form name="FormFilter" id="FormFilter" method="post" action="default.asp?id=@@XMLFile">
	...
	<table  style="font:8pt Arial" border="1" cellPadding="2" cellSpacing="0" width="100%">
		<tr bgColor="#ccccc2">
			 <td title="Items present in Original Order and Confirmation" COLSPAN="15" class="pcontent">&#160;<STRONG>Confirmed Items (Present in original order and confirmation)</STRONG></td>
		 </tr>
		<tr bgColor="#ccccc2">
		...
		<xsl:for-each select="DOCUMENT/UNB/UNH/OLD">
			<xsl:if test="@status='Confirmed'">
				<tr>
					...
					<td align="right" valign="middle" class="pcontent">R<xsl:value-of select="format-number(NELCC, 'DDD,DDD.00', 'staff')"/></td>
