...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/TR/WD-xsl">
   <xsl:template match="/">
     ...
     <table style="font:8pt Arial" border="1" cellPadding="2" cellSpacing="0" width="100%">
      <tr bgColor="#ccccc2">
        <td title="Items present in Original Order but not in Confirmation" style="font:12pt Arial" COLSPAN="11"><xsl:entity-ref name="nbsp" /><STRONG>Unconfirmed Items (present in original order but not in confirmation)</STRONG></td>
      </tr>
      <tr bgColor="#ccccc2">
    ...
    <xsl:for-each select="//OLD[@status='Unconfirmed']">
      <tr>
    ...
    <td align="left" valign="top" title="Discounts or Deals">
      <xsl:choose>
        <xsl:when test="ADJI2[. > ' ']"><xsl:choose>
            <xsl:when test="PERC2[. > ' ']"><xsl:value-of select="ADJI2"/>=<xsl:value-of select="PERC2"/>% </xsl:when>
            <xsl:when test="VALU2[. > ' ']"><xsl:value-of select="ADJI2"/>=R<xsl:value-of select="VALU2"/></xsl:when>
          </xsl:choose>
        </xsl:when>
        <xsl:otherwise></xsl:otherwise>
      </xsl:choose>
