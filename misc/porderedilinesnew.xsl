...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/TR/WD-xsl">
   <xsl:template match="/">
     ...
     <table style="font:8pt Arial" border="1" cellPadding="2" cellSpacing="0" width="100%">
      <tr bgColor="#ccccc2">
        <td title="Items not present in Original Order but present in Confirmation" style="font:12pt Arial" COLSPAN="14"><xsl:entity-ref name="nbsp" /><STRONG>New Items (present in confirmation but not in original order)</STRONG></td>
      </tr>
      <tr bgColor="#ccccc2">
    ...
    <xsl:for-each select="//OLD[@status='New']">
      <tr>
  ...
  <xsl:template match="PROC">
    <td align="left" valign="top" title="Product Barcodes and Supplier Code">
  ...
  <xsl:template match="QNTO">
    <td align="right" valign="middle" title="Quantity confirmed by Supplier"><xsl:value-of select="NROUC"/></td>
  ...
  <xsl:template match="COST">
    <td align="right" valign="middle" title="List Cost per Store Pack before Discounts before VAT"><xsl:apply-templates select="COSPC"/></td>
  </xsl:template>
  ...
  <xsl:template match="NELCC">
    <td align="right" title="Line Cost as confirmed by Supplier">
